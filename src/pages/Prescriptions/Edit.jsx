...
const prescriptionSchema = z.object({
  ...
  const onSubmit = async (data) => {
    const token = localStorage.getItem("token");
    try {
      await axios.patch(`https://ca2-med-api.vercel.app/prescriptions/${id}`, data, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const editForm = (
    <>
      ...
      <CardHeader>
        <CardTitle className="viewCardTitle">Edit Prescription</CardTitle>
        <CardDescription className="viewCardDescription">Update prescription details</CardDescription>
      </CardHeader>
